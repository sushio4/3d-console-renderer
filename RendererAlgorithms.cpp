...
void Renderer::drawLine(const Line &line)
{
    ...
    do
    {
        int z = lerp(beginZ, endZ, fabs(current / (begin - end)));
        ...
        if(isxPrimary)
        {
            if(depthTest(current, currPerp, z) && z <= maxZ)
        ...
        {
            if(depthTest(currPerp, current, z) && z <= maxZ)
...
void Renderer::drawTriangle(const Triangle &triangle)
{
    ...
    for(double x = beginx; x <= endx; x++)
    {
        for(double y = beginy; y <= endy; y++)
        {
            ...
            {
                Vec3 weigths = triangularInterpolation(Vec2(x, y), Triangle(p1, p2, p3));
...
Vec3 Renderer::triangularInterpolation(const Vec2 &p, const Triangle &t)
{
    double a = Vec2::crossProd(t.pt2() - t.pt1(), t.pt3() - t.pt1()).magnitude();
